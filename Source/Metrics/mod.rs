...
//! Includes request latency histograms, resource usage metrics, error rate tracking,
//! thread-safe metric updates with overflow protection, and aggregation validation.
//!
//! ## Responsibilities
//!
//! ### Request Metrics
...
//!
//! Reference: /Volumes/CORSAIR/Developer/macOS/Application/CodeEditorLand/Land/Dependency/Microsoft/Editor/src/vs/workbench/services/telemetry
//!
//! ## Thread Safety
//!
...
//! # TODOs
//!
//! - [DISTRIBUTED TRACING] Integrate with OpenTelemetry for distributed tracing metrics
...
impl MetricsCollector {
    ...
    pub fn RecordRequestSuccess(&self, latency_seconds: f64) {
        self.ValidateAggregation();

        let latency_ms = (latency_seconds * 1000.0) as u64;
    ...
    pub fn RecordRequestFailure(&self, error_type: &str, latency_seconds: f64) {
        self.ValidateAggregation();
    ...
    pub fn UpdateResourceMetrics(&self, memory_bytes: u64, cpu_percent: f64, active_conns: u64, active_threads: u64) {
        self.memory_usage_bytes.store(memory_bytes as i64, Ordering::Relaxed);
        self.cpu_usage_percent.store((cpu_percent * 100.0) as u64, Ordering::Relaxed);
        self.active_connections.store(active_conns, Ordering::Relaxed);
        self.threads_active.store(active_threads, Ordering::Relaxed);
    ...
    pub fn RecordIndexingOperation(&self, entries_indexed: u64) {
        let _ = self.indexing_operations.fetch_add(1, Ordering::Relaxed);
        self.indexing_entries.store(entries_indexed as i64, Ordering::Relaxed);
    }

    /// Record update check
    pub fn RecordUpdateCheck(&self, updates_available: bool) {
    ...
    fn redact_error_type(&self, error_type: &str) -> String {
        let redacted = error_type.to_lowercase();
...
fn min_max_update(min_metric: &AtomicU64, max_metric: &AtomicU64, value: u64) {
    let mut current_min = min_metric.load(Ordering::Relaxed);
    let mut current_max = max_metric.load(Ordering::Relaxed);

    loop {
        if value < current_min {
            match min_metric.compare_exchange_weak(current_min, value, Ordering::Relaxed, Ordering::Relaxed) {
                Ok(_) => break,
                Err(new_min) => current_min = new_min,
            }
        } else if value > current_max {
            match max_metric.compare_exchange_weak(current_max, value, Ordering::Relaxed, Ordering::Relaxed) {
                Ok(_) => break,
...
pub fn GetMetrics() -> &'static MetricsCollector {
