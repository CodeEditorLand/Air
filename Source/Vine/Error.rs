...
impl From<tonic::transport::Error> for VineError {
	fn from(err:tonic::transport::Error) -> Self { VineError::Transport(err.to_string()) }
}

impl From<serde_json::Error> for VineError {
	fn from(err:serde_json::Error) -> Self { VineError::Serialization(err.to_string()) }
}

impl From<std::net::AddrParseError> for VineError {
	fn from(err:std::net::AddrParseError) -> Self { VineError::Transport(format!("Invalid address: {}", err)) }
}

impl From<std::io::Error> for VineError {
	fn from(err:std::io::Error) -> Self { VineError::Internal(err.to_string()) }
}
