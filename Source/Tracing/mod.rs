...
//! correlation ID propagation, trace export capabilities, and sampled tracing for performance.
//!
//! ## Responsibilities
//!
//! ### Trace Generation
...
impl TraceGenerator {
    ...
    pub async fn create_span(
        ...
        let trace_span_count = spans.values()
            .filter(|s| s.trace_id == trace_id)
            .count();

        let config = self.sampling_config.read().await;
        if trace_span_count >= config.max_spans_per_trace {
            log::warn!("[Tracing] Trace {} exceeds max spans ({}), dropping span {}",
                       trace_id, config.max_spans_per_trace, span_id);
    ...
    pub async fn cleanup_old_spans(&self, older_than_ms: Option<u64>) -> Result<usize> {
        ...
        let ttl = older_than_ms.unwrap_or_else(|| {
            tokio::task::block_in_place(|| {
                tokio::runtime::Handle::current().block_on(async {
                    self.sampling_config.read().await.trace_ttl_ms
                })
            })
        });
        ...
        spans.retain(|_, span| {
            span.end_time.map_or(true, |end| now.saturating_sub(end) < ttl)
        });
    ...
    pub async fn get_statistics(&self) -> TraceStatistics {
        ...
        let completed_spans = spans.values()
        ...
        let in_progress_spans = spans.values()
