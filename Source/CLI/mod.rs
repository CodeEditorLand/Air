...
//! ## Responsibilities
//!
//! This module provides the comprehensive command-line interface for the Air daemon, serving as
//! the primary interface for users and administrators to interact with a running Air instance.
//! The CLI is responsible for:
//!
//! - **Command Parsing and Validation**: Parsing command-line arguments, validating inputs,
//!   and providing helpful error messages for invalid commands or arguments
//! - **Command Routing**: Routing commands to the appropriate handlers and executing them
//! - **Configuration Management**: Reading, setting, validating, and reloading configuration
//! - **Status and Health Monitoring**: Querying daemon status, service health, and metrics
//! - **Log Management**: Viewing and filtering daemon and service logs
//! - **Debugging and Diagnostics**: Providing tools for debugging and diagnosing issues
//! - **Output Formatting**: Presenting output in human-readable (table, plain) or machine-readable (JSON) formats
//! - **Daemon Communication**: Establishing and managing connections to the running Air daemon
//! - **Permission Management**: Enforcing security and permission checks for sensitive operations
//!
//! ## VSCode CLI Patterns
//!
//! This implementation draws inspiration from VSCode's CLI architecture:
//! - Reference: /Volumes/CORSAIR/Developer/macOS/Application/CodeEditorLand/Land/Dependency/Microsoft/Dependency/Editor/src/vs/platform/environment/common/environment.ts
...
//!
//! - **Plugin Marketplace Integration**: Add commands for discovering, installing, and managing
//!   plugins from a central marketplace (similar to `code --install-extension`)
//! - **Hot Reload Support**: Implement hot reload of configuration and plugins without daemon restart
//! - **Sandboxing Mode**: Add a sandboxed mode for running commands with restricted permissions
//! - **Interactive Shell**: Implement an interactive shell mode for continuous daemon interaction
//! - **Completion Scripts**: Generate shell completion scripts (bash, zsh, fish) for better UX
//! - **Profile Management**: Support multiple configuration profiles for different environments
//! - **Remote Management**: Add support for managing remote Air instances via SSH/IPC
//! - **Audit Logging**: Add comprehensive audit logging for all administrative actions
//!
...
impl CliParser {
    ...
    fn parse_status(&self, args: &[String]) -> Result<Command, String> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                _ => {
                    return Err(format!("Unknown flag for 'status' command: {}\n\nValid flags are: --service, --verbose, --json", args[i]));
                }
            }
        }
    ...
    fn parse_restart(&self, args: &[String]) -> Result<Command, String> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                _ => {
                    return Err(format!("Unknown flag for 'restart' command: {}\n\nValid flags are: --service, --force", args[i]));
                }
            }
        }
    ...
    fn parse_config(&self, args: &[String]) -> Result<Command, String> {
        if args.is_empty() {
            return Err("config requires a subcommand: get, set, reload, show, validate\n\nUse 'air help config' for more information.".to_string());
        }
        ...
        match subcommand.as_str() {
            ...
            "set" => {
                if args.len() < 3 {
                    return Err("config set requires key and value\n\nExample: air config set grpc.bind_address \"[::1]:50053\"".to_string());
                }
                let key = args[1].clone();
            ...
            _ => Err(format!("Unknown config subcommand: {}\n\nValid subcommands are: get, set, reload, show, validate", subcommand)),
        }
    }
    ...
    fn parse_metrics(&self, args: &[String]) -> Result<Command, String> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                _ => {
                    return Err(format!("Unknown flag for 'metrics' command: {}\n\nValid flags are: --service, --json", args[i]));
                }
            }
        }
    ...
    fn parse_logs(&self, args: &[String]) -> Result<Command, String> {
        ...
        while i < args.len() {
            match args[i].as_str() {
                ...
                "--tail" | "-n" => {
                    if i + 1 < args.len() {
                        tail = Some(args[i + 1].parse::<usize>()
                            .map_err(|_| format!("Invalid tail value '{}': must be a positive integer", args[i + 1]))?);
                ...
                _ => {
                    return Err(format!("Unknown flag for 'logs' command: {}\n\nValid flags are: --service, --tail, --filter, --follow", args[i]));
                }
            }
        }
    ...
    fn parse_debug(&self, args: &[String]) -> Result<Command, String> {
        if args.is_empty() {
            return Err("debug requires a subcommand: dump-state, dump-connections, health-check, diagnostics\n\nUse 'air help debug' for more information.".to_string());
        }
        ...
        match subcommand.as_str() {
            "dump-state" => {
                ...
                while i < args.len() {
                    match args[i].as_str() {
                        ...
                        _ => {
                            return Err(format!("Unknown flag for 'debug dump-state': {}\n\nValid flags are: --service, --json", args[i]));
                        }
                    }
                }
            ...
            "dump-connections" => {
                ...
                while i < args.len() {
                    match args[i].as_str() {
                        ...
                        _ => {
                            return Err(format!("Unknown flag for 'debug dump-connections': {}\n\nValid flags are: --format", args[i]));
                        }
                    }
                }
            ...
            "health-check" => {
                ...
                while i < args.len() {
                    match args[i].as_str() {
                        ...
                        _ => {
                            return Err(format!("Unknown flag for 'debug health-check': {}\n\nValid flags are: --service, --verbose", args[i]));
                        }
                    }
                }
            ...
            "diagnostics" => {
                ...
                while i < args.len() {
                    match args[i].as_str() {
                        ...
                        _ => {
                            return Err(format!("Unknown flag for 'debug diagnostics': {}\n\nValid flags are: --basic, --extended, --full", args[i]));
                        }
                    }
                }
            ...
            _ => Err(format!("Unknown debug subcommand: {}\n\nValid subcommands are: dump-state, dump-connections, health-check, diagnostics", subcommand)),
        }
    }
    ...
    fn validate_service_name(service: &Option<String>) -> Result<(), String> {
        if let Some(s) = service {
            ...
            if !s.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
                return Err("Service name can only contain alphanumeric characters, hyphens, and underscores".to_string());
            }
        }
        Ok(())
    }
...
impl DaemonClient {
    ...
    fn get_mock_services(&self) -> HashMap<String, ServiceStatus> {
        ...
        services.insert("authentication".to_string(), ServiceStatus {
        ...
        services.insert("updates".to_string(), ServiceStatus {
        ...
        services.insert("plugins".to_string(), ServiceStatus {
    ...
    fn get_mock_service_metrics(&self) -> HashMap<String, ServiceMetrics> {
        ...
        metrics.insert("authentication".to_string(), ServiceMetrics {
            name: "authentication".to_string(),
            requests_total: 500,
        ...
        metrics.insert("updates".to_string(), ServiceMetrics {
            name: "updates".to_string(),
            requests_total: 300,
...
impl CliHandler {
    ...
    pub fn execute(&mut self, command: Command) -> Result<String, String> {
        ...
        match command {
            Command::Status { service, verbose, json } => {
                self.handle_status(service, verbose, json)
            }
            Command::Restart { service, force } => {
                self.handle_restart(service, force)
            }
            Command::Config(config_cmd) => {
                self.handle_config(config_cmd)
            }
            Command::Metrics { json, service } => {
                self.handle_metrics(json, service)
            }
            Command::Logs { service, tail, filter, follow } => {
                self.handle_logs(service, tail, filter, follow)
            }
            Command::Debug(debug_cmd) => {
                self.handle_debug(debug_cmd)
            }
            Command::Help { command } => {
                Ok(OutputFormatter::format_help(command.as_deref(), "0.1.0"))
            }
            Command::Version => {
                Ok("Air 🪁 v0.1.0".to_string())
            }
        }
    }
    ...
    fn handle_logs(&self, service: Option<String>, tail: Option<usize>, filter: Option<String>, follow: bool) -> Result<String, String> {
        ...
        for entry in logs {
            output.push_str(&format!("[{}] {} - {}\n", entry.timestamp.format("%Y-%m-%d %H:%M:%S"), entry.level, entry.message));
        }
