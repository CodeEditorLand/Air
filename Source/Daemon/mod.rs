...
//!
//! This module provides comprehensive daemon lifecycle management for the Air daemon service,
//! responsible for managing background processes in the Land code editor ecosystem.
//!
...
impl DaemonManager {
    ...
    pub async fn AcquireLock(&self) -> Result<()> {
        ...
        if let Some(parent) = self.pid_file_path.parent() {
            ...
            #[cfg(unix)]
            {
                ...
                fs::set_permissions(parent, perms).map_err(|e| {
                    AirError::FileSystem(format!("Failed to set directory permissions: {}", e))
                })?;
            }
        }
        ...
        fs::write(&temp_dir, &temp_file_content).map_err(|e| {
            AirError::FileSystem(format!("Failed to write temporary PID file: {}", e))
        })?;
                ...
                Ok(output) => {
                    if output.status.success() {
                        ...
                        stdout.lines().skip(1).any(|line| {
                            line.contains("air") || line.contains("daemon")
                        })
                    } else {
                        false
                    }
    ...
    async fn CleanupStalePidFile(&self) -> Result<()> {
        ...
        let content = fs::read_to_string(&self.pid_file_path).map_err(|e| {
    ...
    pub async fn IsRunning(&self) -> bool {
...
"#,
    ...
    /// TODO: Integrate with winsvc crate for actual Windows Service registration
...
"#,
    ...
    async fn InstallSystemdService(&self) -> Result<()> {
        ...
        if !service_file_content.contains("[Unit]") || !service_file_content.contains("[Service]") {
            return Err(AirError::Configuration(
                "Generated service file is invalid".to_string()
            ));
        }
        
        // Write to temporary file first
        fs::write(&temp_path, &service_file_content).map_err(|e| {
            AirError::FileSystem(format!("Failed to write temporary service file: {}", e))
        })?;
        ...
        #[cfg(unix)]
        {
            ...
            fs::set_permissions(&service_file_path, perms).map_err(|e| {
    ...
    async fn InstallLaunchdService(&self) -> Result<()> {
        ...
        if !service_file_content.contains("<?xml") || !service_file_content.contains("<!DOCTYPE plist") {
            return Err(AirError::Configuration(
                "Generated plist file is invalid".to_string()
            ));
        }
        
        // Write to temporary file first
        fs::write(&temp_path, &service_file_content).map_err(|e| {
            AirError::FileSystem(format!("Failed to write temporary plist file: {}", e))
        })?;
    ...
    async fn InstallWindowsService(&self) -> Result<()> {
        ...
        fs::create_dir_all(service_dir).map_err(|e| {
            AirError::FileSystem(format!("Failed to create service directory: {}", e))
        })?;
