...
//!
//! Provides comprehensive structured logging with JSON output, request ID propagation,
//! context-aware logging, log rotation, sensitive data filtering, and validation.
//!
//! ## Responsibilities
//!
//! ### Structured Logging
...
impl LogRotationConfig {
    ...
    pub fn Validate(&self) -> Result<()> {
        ...
        if self.max_file_size_bytes > 10 * 1024 * 1024 * 1024 {
            // Max 10 GB
            return Err("max_file_size_bytes cannot exceed 10 GB".into());
        }
        if self.max_files == 0 {
            return Err("max_files must be greater than 0".into());
        }
        if self.max_files > 365 {
            // Max 1 year retention
            return Err("max_files cannot exceed 365".into());
        }
        Ok(())
    }
...
pub struct SensitiveDataConfig {
    ...
    pub include_standard_patterns: bool,
}
...
impl Default for SensitiveDataConfig {
    fn default() -> Self {
        Self {
            ...
            include_standard_patterns: true,
        }
    }
}
...
pub fn GetLogContext() -> Option<LogContext> {
...
impl LogManager {
    ...
    fn Rotate(&self) -> Result<()> {
        ...
        if let Some(ref file_path) = *current_file {
            ...
            let rotated_path = format!("{}.{}.log", file_path.display(), timestamp);

            std::fs::rename(file_path, &rotated_path)?;

            // Compress if enabled
            if self.config.compress {
                self.CompressFile(&rotated_path)?;
            }
    ...
    pub fn Initialize(&self) -> Result<()> {
        ...
        if self.json_output {
            ...
            if let Some(ref log_path) = self.log_file_path {
                let log_dir = std::path::Path::new(log_path).parent()
...
pub fn GetLogger() -> &'static ContextLogger {
