...
use crate::{
	AirError,
	ApplicationState::{
		ConnectionHealthReport::ConnectionHealthReport,
...
pub struct Struct {
	...
	pub Configuration:Arc<AirConfig>,
...
impl Struct {
	...
	pub async fn New(Configuration:Arc<AirConfig>) -> Result<Self> {
		let State = Self {
			Configuration,
	...
	pub async fn GetConfiguration(&self) -> Arc<AirConfig> { self.Configuration.clone() }
