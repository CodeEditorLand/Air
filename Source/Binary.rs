...
//! communicating via the Vine (gRPC) protocol to offload tasks like updates, downloads,
...
//!
//! - **Wind** (Effect-TS): Provides functional programming patterns and type-safe effects
//!   that Air uses for predictable state management and error handling
//!
//! - **Cocoon** (NodeJS host): The Node.js runtime environment for frontend/web components.
//!   Air coordinates with Cocoon through the Vine protocol to deliver web assets and
//!   perform frontend build operations. Uses port 50052.
//!
//! - **Mountain** (Tauri bundler): The main desktop application bundle that packages
//!   the Rust backend with the Electron/Node.js frontend. Mountain receives work from
//!   Air through Vine (gRPC) and orchestrates the bundling process.
//!
//! - **Vine** (gRPC protocol): The communication layer connecting all components.
//!   Air hosts the Vine gRPC server on port 50053, receiving requests from Mountain
//!   and responding with results from background operations.
//!
...
///
/// Sets up structured logging with support for JSON output (useful for production)
/// and file-based logging (useful for debugging). Environment variables:
///
    ...
    let json_output = match std::env::var("AIR_LOG_JSON") {
        Ok(val) if !val.is_empty() => {
            ...
            if normalized != "true" && normalized != "false" {
                eprintln!("Warning: Invalid AIR_LOG_JSON value '{}', expected 'true' or 'false'. Using default: false", val);
                false
            } else {
                normalized == "true"
            }
        ...
        if path.is_empty() {
            ...
            if let Some(parent) = std::path::PathBuf::from(&path).parent() {
                if parent.as_os_str().is_empty() {
                    ...
                    eprintln!("Warning: Log file directory does not exist: {}. Logging to stdout only.", parent.display());
                    None
                }
            } else {
                Some(path)
    ...
    match export_result {
        Ok(Ok(metrics_text)) => {
            ...
            if metrics_text.len() > 10_000_000 {
                error!("[Metrics] Exported metrics unreasonably large (size: {} bytes)", metrics_text.len());
