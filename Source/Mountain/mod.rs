...
//! implementation focuses on establishing unencrypted connections for development
...
pub struct TlsConfig {
    /// Path to the CA certificate file (optional, uses system defaults if not provided)
...
pub fn create_tls_client_config(
    ...
    // Load client certificate and key for mTLS (must be done before building config)
    let (client_certs, client_key) = if tls_config.client_cert_path.is_some() && tls_config.client_key_path.is_some() {
        ...
        let cert_file = File::open(cert_path)
            .map_err(|e| format!("Failed to open client certificate file: {}", e))?;
    ...
    // Build the client config - rustls 0.23 returns ClientConfig directly
    let mut config = if let Some((certs, key)) = client_certs {
    ...
    // Note: Certificate verification can only be disabled during the config build phase
    // The current rustls API doesn't support disabling verification after building
    // If verification needs to be disabled, use NoServerAuthVerifier during build
...
impl MountainClientConfig {
    ...
    /// - `MOUNTAIN_CONNECTION_TIMEOUT_SECS`: Connection timeout in seconds (default: 5)
    /// - `MOUNTAIN_REQUEST_TIMEOUT_SECS`: Request timeout in seconds (default: 30)
    ...
    /// - `MOUNTAIN_VERIFY_CERTS`: Verify certificates (default: true, set to "0" or "false" to disable)
    ///
    /// # Returns
    /// New MountainClientConfig instance loaded from environment
    pub fn from_env() -> Self {
        let address = env::var("MOUNTAIN_ADDRESS")
            .unwrap_or_else(|_| DEFAULT_MOUNTAIN_ADDRESS.to_string());
...
impl MountainClient {
    ...
    pub async fn connect(config: MountainClientConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if let Some(tls_config) = &config.tls_config {
            ...
            let client_config = create_tls_client_config(tls_config)
                .map_err(|e| {
                    error!("Failed to create TLS client configuration: {}", e);
                    format!("TLS configuration error: {}", e)
                })?;

            // Create tonic TLS configuration
            let mut tonic_tls_config = tonic::transport::ClientTlsConfig::new();

            // Set server name if provided for SNI
            if let Some(server_name) = &tls_config.server_name {
                tonic_tls_config = tonic_tls_config.domain_name(server_name.clone());
                info!("Setting server name for SNI: {}", server_name);
            } else {
                tonic_tls_config = tonic_tls_config.domain_name("localhost".to_string());
            }

            // Use tonic's TLS configuration with our ClientConfig
            tonic_tls_config = tonic_tls_config.tls_config(client_config);

            let channel = endpoint
                .connect_with_connector(tonic_tls_config.connect())
                .await
