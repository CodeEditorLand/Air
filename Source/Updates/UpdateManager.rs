...
//!
//! Coordinates update checking, downloading, verification,
//! staged installation, automatic rollback, and telemetry.
...
impl UpdateManager {
	...
	async fn FetchUpdateInfo(&self) -> Result<Option<UpdateInfo>> {
		...
		let retry_policy = crate::Resilience::Retry::RetryPolicy {
			MaxRetries:3,
		...
		let _retry_manager = crate::Resilience::Retry::RetryManager::new(retry_policy.clone());

		let circuit_breaker = crate::Resilience::CircuitBreaker::CircuitBreaker::new(
			"updates".to_string(),
		...
		loop {
			...
			if circuit_breaker.GetState().await == crate::Resilience::CircuitState::CircuitState::Open {
